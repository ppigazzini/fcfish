...
do_pgo() {
  info "PGO build of $BIN ($FCFISH_ARCH): instrument -> profile bench -> rebuild"
  mkdir -p build
  ...
  info "phase 1/3: instrumented build -> build/fcfish-instr"
  "$CC" "${CFLAGS_COMMON[@]}" "${CFLAGS_RELEASE[@]}" -fprofile-generate="$profdir" \
    -o build/fcfish-instr "${SOURCES[@]}" "${LIBS[@]}"
  ...
  ( cd "$ROOT/$RESOURCES_DIR" \
      && LLVM_PROFILE_FILE="$ROOT/$profdir/fcfish-%p.profraw" \
         "$ROOT/build/fcfish-instr" bench > /dev/null 2>&1 )
...
measure_instructions() {
  local counter="${TMPDIR:-/tmp}/fcfish_perf_counters"
...
do_perf_budget() {
  ...
  budget=$(grep -v '^#' "$PERF_BUDGET_GOLDEN" 2>/dev/null | awk -v a="$FCFISH_ARCH" '$1==a{print $2}')
  if [[ -z $budget ]]; then
    info "perf-budget: no budget recorded for arch '$FCFISH_ARCH'."
    info "Record one from a known-good build: FCFISH_ARCH=$FCFISH_ARCH ./build.sh perf-budget-update"
    return 127
  fi

  # Ceiling = budget * (1 + TOL). A regression INFLATES instructions; a drop is a win.
  awk -v a="$actual" -v b="$budget" -v tol="$PERF_BUDGET_TOL" -v arch="$FCFISH_ARCH" 'BEGIN{
...
do_perf_budget_update() {
  ...
  [[ -f $PERF_BUDGET_GOLDEN ]] || {
    { echo "# fcfish retired-instruction budget: median count on 'bench $PERF_BUDGET_BENCH',"
  ...
  awk -v a="$FCFISH_ARCH" -v n="$actual" '
    /^#/ { print; next }
    $1==a { next }
    { print }
    END { print a, n }' "$PERF_BUDGET_GOLDEN" > "$tmp" && mv "$tmp" "$PERF_BUDGET_GOLDEN"
  green "perf-budget golden for '$FCFISH_ARCH' set to $actual instructions (bench $PERF_BUDGET_BENCH, $nodes nodes)"
}
...
usage: ./build.sh <step> [args]
  ...
  pgo                profile-guided build (instrument, bench, rebuild) -> build/fcfish
