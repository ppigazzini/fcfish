#!/usr/bin/env bash
# The finish-line gate: fcfish's bench must equal a PRISTINE upstream build's bench
# at the pinned SHA.
#
...
# anchor; this is the one that decides whether fcfish is a Stockfish clone.
#
# Usage:  upstream_parity.sh [fcfish-binary] [sha]
set -euo pipefail
...
[[ -x $BIN ]] || { red "no fcfish binary at $BIN — run ./build.sh first"; exit 2; }
...
# does fcfish (deliberately — that stream choice is faithful, not a bug).
# Run each engine from a directory that HOLDS ITS NET. The oracle keeps one beside
# its binary; fcfish keeps one in resources/ (build.sh RESOURCES_DIR). Running
# fcfish from the repo root instead benched the classical fallback and reported a
# false divergence -- and once fcfish started refusing to run without a net, the
...
if [[ -n $want && $got == "$want" ]]; then
  green "BIT-EXACT — fcfish reproduces upstream's bench signature"
  exit 0
fi
