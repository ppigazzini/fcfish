...
A golden is not automatically a reference. When it is regenerated from fcfish, it
is a **photograph of fcfish** — and it will pin a defect exactly as faithfully as it
...
generated from fcfish and both pinned real divergences for as long as they existed:

- `board.golden` recorded a `d` output with **no `Checkers:` line**, which upstream
  always prints, and recorded fcfish silently accepting the illegal move `b4b6`
  and continuing.
...
Regenerate an oracle-derived golden from the oracle. Never from fcfish: that
...
3-man set — there is no fcfish-derived path to that golden at all.

```sh
NORM=$(sed -n '/^normalize()/,/^}/p' build.sh)
cd ../.fcfish-upstream-oracle/src && eval "$NORM"
{ ./stockfish < /home/usr00/_git/fcfish/tools/cases/<case>.uci 2>&1; printf 'exit=%d\n' "$?"; } \
  | normalize > /home/usr00/_git/fcfish/tools/<case>.golden
```

## `handshake`: the one legitimate substitution

`handshake` is derived from the oracle and then has exactly one line rewritten,
because exactly one line cannot be compared: fcfish is not named Stockfish, and
...
diff <(./build/fcfish < tools/cases/search.uci 2>&1 | normalize) \
     <(cd ../.fcfish-upstream-oracle/src && ./stockfish < "$W/tools/cases/search.uci" 2>&1 | normalize)
```
...
`normalize` elides volatile fields (time, nps) and **drops** upstream lines fcfish
...
drive_oracle() {
  { while IFS= read -r l; do printf '%s\n' "$l"; case "$l" in go*) sleep 5;; esac; done < "$1"
    sleep 1; } | (cd ../.fcfish-upstream-oracle/src && ./stockfish) 2>&1
}
{ drive_oracle tools/cases/<case>.uci; printf 'exit=0\n'; } | normalize \
  | sed -E 's/^(fcfish|Stockfish) [^ ]+ by .*/<engine banner>/' > tools/<case>.golden
```

fcfish's own `go` is synchronous, so the gate itself may pipe — only the oracle
side needs this.
