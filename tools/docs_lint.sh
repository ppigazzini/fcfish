...
# Docs here legitimately name paths in THREE repos: fcfish's own tree, the zfish
# port source, and the Stockfish golden. A path is a valid claim if it resolves in
# any of them. Checking only fcfish would flag every upstream citation -- and the
