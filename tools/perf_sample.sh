...
#   PS_UCI='uci\nposition fen ...\ngo movetime 20000' ../tools/perf_sample.sh ../build/fcfish 40000
#   # drill into a hot symbol by instruction offset (map with objdump -d):
#   PS_FOCUS=evaluate_side.75 PS_UCI='...' ../tools/perf_sample.sh ../build/fcfish 30000
set -u

SELF_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SRC="$SELF_DIR/perf_sample.c"
BIN="${TMPDIR:-/tmp}/fcfish_perf_sample"
