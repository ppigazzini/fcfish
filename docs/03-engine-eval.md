...
fcfish reports the failure, leaves `NetLoaded` false, and keeps playing on the
...
from fcfish would convert a red gate into a recorded bug; regenerate it from the
...
- **The classical fallback is still in the tree**, and its deletion waits on the
  point where a netless run is no longer something fcfish needs to support.
- **Bit-exactness with upstream is not reached**, and the evaluation is only one of
  ...
  [`../tools/signature.golden`](../tools/signature.golden) is fcfish's *current*
