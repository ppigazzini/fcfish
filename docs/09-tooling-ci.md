...
| `wp` | [`../tools/framac/wp.sh`](../tools/framac/wp.sh) runs WP + Z3 over the ACSL contracts in `types.h` and `tt.c` | **deductive** (symbolic, all-inputs) proof of the non-bitwise helper contracts — `piece_value` (no out-of-bounds read, exact value), `mate_in`/`mated_in` (exact value, score bounds), and `depth_saturating_sub` (the TT depth clamp never wraps a shallow entry deep). The bitwise codecs stay with `eva` because WP encodes shifts as nonlinear goals no SMT prover discharges. Fails on any unproved goal; exits **127** without the opam switch or Z3 |
