...
| `eva` | [`../tools/framac/eva.sh`](../tools/framac/eva.sh) runs Eva over **seven harnesses** (board/NNUE, slider, DirtyThreat, movegen, FEN, platform NumaConfig, platform memory), each feeding its functions their full valid intervals — see [10-frama-c.md](10-frama-c.md) for the roster | runtime safety **and** codec correctness: **0 alarms** proves no out-of-range shift, signed overflow or out-of-bounds access, and the case-split round-trips prove the square/piece/move encoders and all five `dirty_threat_make` feature-word fields decode back exactly (the middle shift-and-mask field under Eva's bitwise domain); the NNUE clipped-ReLU output lands in the quantized `[0, 127]` range; `attacks_bb`'s leaper **and** slider paths are proved in-bounds (the magic index from the shift/block-size invariant alone, no `attacks_init`); the movegen generators never overrun their buffer; the `pos_set_reason` FEN parser never writes off the board on malformed input; and across the platform layer the NumaPolicy string parser, the `suggests_binding_threads`/`distribute_threads` placement decisions and `memory.c`'s `posix_memalign` aligned allocator are proved runtime-safe. The realloc-grown numa insertion and the mmap page allocator are the documented boundaries neither analyser reaches. Fails on any alarm or unproven assertion; exits **127** without the opam switch |
| `wp` | [`../tools/framac/wp.sh`](../tools/framac/wp.sh) runs WP + Z3 over the ACSL contracts in `types.h`, `score.c`, `history.h`, `history.c`, `search_common.c`, `tt.c` and `ucioption.c` | **deductive** (symbolic, all-inputs) proof of the non-bitwise helper contracts — `piece_value` (no out-of-bounds read, exact value), `mate_in`/`mated_in` (exact value, score bounds), `score_classify` (the mate/TB classifier returns a valid kind and its `-value` negation never overflows), `stats_update` (the history gravity update keeps `|entry| <= D`, so int16 storage never overflows), `stat_malus` (the search malus stays `<= 2244`, inside every history clamp band, with no overflow), `correction_history_bonus` (lands in `[-265, 265]`, inside the correction clamp, with no overflow), `prior_bonus_scale`/`prior_scaled_bonus_base`/`tt_move_history_depth_bonus`/`tt_move_history_match_bonus` (the search-move bonuses stay within their stated bounds, all overflow-free), the depth margins `razor_margin`/`null_move_reduction`/`move_count_limit` (bounded for a search depth in `[0, MAX_PLY]`, with `move_count_limit`'s `2 - improving` divisor proved never zero), the Value-band helpers `to_corrected_static_eval` (clamped clear of the decisive band, so it can never masquerade as a proven win or loss), `futility_return`/`qsearch_stand_pat_blend`/`qsearch_fail_high_blend`/`qsearch_futility_base` (kept inside the Value band, overflow-free), `store` (the option-table string copy never overruns its fixed buffer, however long the source) and `depth_saturating_sub` (the TT depth clamp never wraps a shallow entry deep). The bitwise codecs stay with `eva` because WP encodes shifts as nonlinear goals no SMT prover discharges. Fails on any unproved goal; exits **127** without the opam switch or Z3 |
...
| --- | --- | --- |
| fcfish | 3.016e9 | **0.890x** |
| Stockfish | 3.388e9 | 1.000 |

**Whole-process, fcfish executes fewer instructions than upstream** — and against
the Zig port, [08-idiomatic-c.md](08-idiomatic-c.md)'s finding pays off: Clang
auto-vectorizes the integer loops the Zig toolchain left scalar, so fcfish never
...
startup is cheaper in fcfish. The **per-tier search** picture, once startup is
...
| tier | fcfish / Stockfish instructions | IPC (fcfish/SF), startup-clean |
| --- | --- | --- |
| sse41 | 1.115 | ~1.0 |
| avx2 | 1.121 | ~1.0 |
| native / vnni512 | 1.139 | ~1.0 |

**On real search work fcfish executes ~12–14% more instructions than upstream at every
...
it is startup, which is cheaper in fcfish (see below).

**IPC is at PARITY once startup is removed** — not below 1. The earlier "IPC < 1 on
every tier" reading was the net-load confound: fcfish loads the ~90 MB net in ~245 ms
vs upstream's ~458 ms, and `perf_counters`/`nps_ab` arm counters before exec, so a
whole-process shallow read credits fcfish with an IPC/cache "advantage" that is really
cheaper startup. Startup-clean (the delta method, or a deep `bench 16 1 13/14`),
fcfish's per-node IPC is ~1.0 at every depth and tier. **So the entire deficit is
