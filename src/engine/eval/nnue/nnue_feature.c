...
void nnue_full_append_changed(uint8_t perspective,
    ...
    // nothing to recover. Hidden from the analyser, whose kernel rejects the clang loop
    // pragma and never vectorizes regardless (as tt.c hides its 128-bit mul_hi64).
#ifndef __FRAMAC__
